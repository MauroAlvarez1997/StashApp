...
class Transaction(db.Model, UserMixin):
  ...
  recipient = db.relationship('User', overlaps="user_in", back_populates='transaction_in', foreign_keys=[recipient_id])
  # User.recipient = db.relationship('Transaction', foreign_keys=[recipient_id])
  sender = db.relationship('User', overlaps="user_out", back_populates='transaction_out', foreign_keys=[sender_id])
